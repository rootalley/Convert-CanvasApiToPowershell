...
function Get-UniqueCmdletName($Method) {
    ...
    $Terms.Add($Verb) | Out-Null
    $Terms.Add("-Canvas") | Out-Null

    $Split = $Method.path.Split("/")

    for ($i = 1; $i -lt $Split.Count; $i++) {
        if ($Split[$i] -match"{.*}") {
            $SplitTitleCase = ConvertTo-TitleCase ($Split[$i] -replace "[{}]","") "_"
            $Params.Add($SplitTitleCase) | Out-Null
        } elseif (-not [string]::IsNullOrWhiteSpace($Split[$i])) {
            if("V1" -ne $Split[$i]) {
                $SplitTitleCase = ConvertTo-TitleCase $Split[$i] "_"
                $Words.Add($SplitTitleCase) | Out-Null
            }
        }
    }

    for ($i = 0; $i -lt $Params.Count; $i++) {
        $Index = ($Words.TrimEnd("s")).IndexOf($Params[$i].TrimEnd("Id"))
        if (($Index -ge 0) -and ($Index -ne ($Words.Count - 1))) {
            $Words.Remove($Words[$Index])
        }
    }

    foreach ($Word in $Words) {
        $Terms.Add($Word) | Out-Null
    }

    if ($Params.Count -gt 0) {
        $Terms.Add("By" + ($Params -join "And")) | Out-Null
    }
